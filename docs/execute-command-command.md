...
- `--output-https <https://host:port/path>` — top-level global option to POST formatted output to the helper API over HTTPS
...
./embedded_linux_audit --insecure --output-https https://192.168.1.50:5443 linux execute-command "dmesg | tail"
