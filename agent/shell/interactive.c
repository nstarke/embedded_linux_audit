...
static void interactive_usage(const char *prog)
{
	printf("Interactive mode commands:\n"
	       ...
	       "  usb list | reset <DEVICE_INDEX>\n"
	       "  usb port <list|reset|power-cycle> [PORT_INDEX]\n"
	       ...
	       "  bios orom <pull|list>\n"
	       "\n"
	       "Kernel-backed hardware commands require loaded ela_kmod,\n"
	       "/dev/ela_physmem, and CAP_SYS_RAWIO. Use index=N from list\n"
	       "as DEVICE_INDEX. eMMC requires Linux 6.9 or newer.\n"
	       "\n"
	       "Examples:\n"
