...
/*
 * The wolfSSL HTTPS backend lives in http_client_wolfssl.c, not here: wolfSSL is
 * built with --enable-opensslextra, so its headers alias the OpenSSL type and
 * function names onto the WOLFSSL_* ones and cannot share a translation unit
 * with the real OpenSSL headers this file needs. Do not include any wolfSSL
 * header from here.
 */
#ifdef SHA256
#undef SHA256
			     ...
			     int *status_out)
{
	...
	if (ela_http_choose_https_backend(isa_is_powerpc_family(ela_detect_isa())) ==
		...
		return ela_http_wolfssl_post(&parsed, uri, data, len, content_type,
				    ...
				    size_t errbuf_len)
{
	...
	if (ela_http_choose_https_backend(isa_is_powerpc_family(ela_detect_isa())) ==
		...
		return ela_http_wolfssl_get_to_file(&parsed, uri, output_path, insecure,
