...
#include "linux_list_files_util.h"
...
static int default_append_output(struct output_buffer *buf, const char *text)
{
	return output_buffer_append(buf, text);
}
...
static const struct ela_list_files_prepare_ops default_prepare_ops = {
	.parse_permissions_filter_fn = ela_parse_permissions_filter,
	.parse_http_output_uri_fn = default_parse_http_output_uri,
	.connect_tcp_ipv4_fn = default_connect_tcp_ipv4,
	.lstat_fn = lstat,
};

static const struct ela_list_files_run_ops default_run_ops = {
	.lstat_fn = lstat,
...
static int emit_path(const struct ela_list_files_request *request,
		     ...
		     const char *path)
{
	...
	if (request->output_sock >= 0 &&
	    ops->send_all_fn(request->output_sock, (const uint8_t *)line, (size_t)line_len) < 0)
		return -1;

	if (request->output_uri)
		return ops->append_output_fn(buf, line);

	return ops->write_stdout_fn(line, (size_t)line_len);
}

static void report_list_error(const struct ela_list_files_request *request,
			      const struct ela_list_files_run_ops *ops,
			      const char *fmt,
			      const char *path)
{
...
static int list_files_recursive(const struct ela_list_files_request *request,
				const struct ela_list_files_run_ops *ops,
				struct output_buffer *buf,
				const char *dir_path)
{
	...
	if (!dir) {
		report_list_error(request, ops, "Cannot open directory %s: %s\n", dir_path);
		return -1;
	}
	...
	while ((de = readdir(dir)) != NULL) {
		...
		if (ops->lstat_fn(child, &st) != 0) {
			report_list_error(request, ops, "Cannot stat %s: %s\n", child);
			continue;
		}
		...
		if (S_ISDIR(st.st_mode)) {
			if (!request->recursive)
				continue;
			if (list_files_recursive(request, ops, buf, child) != 0) {
				closedir(dir);
				return -1;
			}
			continue;
		...
		if (!ela_list_files_entry_matches_filters(&st, &request->filters))
			continue;

		if (emit_path(request, ops, buf, child) != 0) {
			closedir(dir);
			return -1;
		}
	}
...
int ela_list_files_prepare_request(int argc, char **argv,
				   ...
				   char *errbuf, size_t errbuf_len)
{
	const struct ela_list_files_prepare_ops *effective_ops = ops ? ops : &default_prepare_ops;
	...
	if (!out->dir_path || out->dir_path[0] != '/') {
		set_errbuf(errbuf, errbuf_len, "list-files requires an absolute directory path");
		return 2;
	}

	if (permissions_arg &&
	    effective_ops->parse_permissions_filter_fn(permissions_arg, &out->filters.permissions) != 0) {
		set_errbuf(errbuf, errbuf_len, "Invalid --permissions value: %s", permissions_arg);
		return 2;
	}

	if (user_arg) {
		if (resolve_user_filter(user_arg, &out->filters.uid) != 0) {
			set_errbuf(errbuf, errbuf_len, "Invalid --user value: %s", user_arg);
			return 2;
		}
		out->filters.user_set = true;
	}

	if (group_arg) {
		if (resolve_group_filter(group_arg, &out->filters.gid) != 0) {
			set_errbuf(errbuf, errbuf_len, "Invalid --group value: %s", group_arg);
			return 2;
		}
		out->filters.group_set = true;
	}
	...
	if (!S_ISDIR(st.st_mode)) {
		set_errbuf(errbuf, errbuf_len, "list-files requires a directory path: %s", out->dir_path);
		return 2;
	}
...
int ela_list_files_run(const struct ela_list_files_request *request,
		       ...
		       char *errbuf, size_t errbuf_len)
{
	const struct ela_list_files_run_ops *effective_ops = ops ? ops : &default_run_ops;
	struct output_buffer buf = {0};
	char *upload_uri = NULL;
	char post_err[256];
	int ret = 0;

	if (!request || !request->dir_path)
		return 1;

	if (list_files_recursive(request, effective_ops, &buf, request->dir_path) != 0) {
		ret = 1;
		goto out;
	}

	if (request->output_uri) {
		upload_uri = effective_ops->build_upload_uri_fn(request->output_uri, "file-list",
								 request->dir_path);
		if (!upload_uri) {
