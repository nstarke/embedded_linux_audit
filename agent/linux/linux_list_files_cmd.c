...
int linux_list_files_scan_main(int argc, char **argv)
{
	...
	ret = ela_list_files_prepare_request(argc, argv, &env, NULL, &request, errbuf, sizeof(errbuf));
	if (ret != 0) {
		fprintf(stderr, "%s\n", errbuf);
		if (ret == 2 &&
		    strstr(errbuf, "Use only one of --output-http or --output-https") == NULL &&
		    strstr(errbuf, "Invalid/failed output target") == NULL &&
		    strstr(errbuf, "list-files requires a directory path:") == NULL &&
		    strstr(errbuf, "Invalid --user value:") == NULL &&
		    strstr(errbuf, "Invalid --group value:") == NULL &&
		    strstr(errbuf, "Invalid --permissions value:") == NULL)
			usage(argv[0]);
		return ret;
	}
	...
	ret = ela_list_files_run(&request, NULL, errbuf, sizeof(errbuf));
	if (ret != 0 && errbuf[0])
		fprintf(stderr, "%s\n", errbuf);
	return ret;
}
