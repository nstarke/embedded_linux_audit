...
static const struct ela_grep_prepare_ops default_prepare_ops = {
	.parse_http_output_uri_fn = default_parse_http_output_uri,
	.connect_tcp_ipv4_fn = default_connect_tcp_ipv4,
	.lstat_fn = lstat,
};

static const struct ela_grep_run_ops default_run_ops = {
	.fopen_fn = fopen,
	.getline_fn = getline,
	.fclose_fn = fclose,
	.lstat_fn = lstat,
	.format_grep_match_record_fn = ela_format_grep_match_record,
...
static void report_grep_error(const struct ela_grep_request *request,
			      ...
			      const char *path)
{
	...
	if (ops->http_post_log_message_fn(request->output_uri, msg,
...
static int emit_match(const struct ela_grep_request *request,
		      const struct ela_grep_run_ops *ops,
		      ...
		      const char *line)
{
	...
	if (!request || !ops || !path || !line)
		return -1;

	if (ops->format_grep_match_record_fn(&out, path, line_no, line) != 0)
		goto out;

	if (request->output_sock >= 0 &&
	    ops->send_all_fn(request->output_sock, (const uint8_t *)out.data, out.len) < 0)
		goto out;

	if (request->output_uri) {
		if (ops->append_output_fn(capture_buf, out.data, out.len) != 0)
			goto out;
	} else if (ops->write_stdout_fn(out.data, out.len) != 0) {
		goto out;
	}
...
static int grep_file(const struct ela_grep_request *request,
		     const struct ela_grep_run_ops *ops,
		     struct output_buffer *capture_buf,
		     const char *file_path)
{
	...
	fp = ops->fopen_fn(file_path, "r");
	if (!fp)
		return -1;

	while (ops->getline_fn(&line, &line_cap, fp) >= 0) {
		line_no++;
		if (!strstr(line, request->search))
			continue;
		if (emit_match(request, ops, capture_buf, file_path, line_no, line) != 0) {
			ret = -1;
			break;
		}
	}
...
static int grep_directory(const struct ela_grep_request *request,
			  const struct ela_grep_run_ops *ops,
			  struct output_buffer *capture_buf,
			  const char *dir_path)
{
	...
	if (!dir) {
		report_grep_error(request, ops, "Cannot open directory %s: %s\n", dir_path);
		return -1;
	}
	...
	while ((de = readdir(dir)) != NULL) {
		...
		if (ops->lstat_fn(child, &st) != 0) {
			report_grep_error(request, ops, "Cannot stat %s: %s\n", child);
			continue;
		}

		if (S_ISDIR(st.st_mode)) {
			if (request->recursive &&
			    grep_directory(request, ops, capture_buf, child) != 0) {
				closedir(dir);
				return -1;
			}
			continue;
		...
		if (grep_file(request, ops, capture_buf, child) != 0)
			report_grep_error(request, ops, "Cannot read file %s: %s\n", child);
	}
...
int ela_grep_prepare_request(int argc, char **argv,
			     ...
			     char *errbuf, size_t errbuf_len)
{
	const struct ela_grep_prepare_ops *effective_ops = ops ? ops : &default_prepare_ops;
	...
	if (!out->search || !*out->search) {
		set_errbuf(errbuf, errbuf_len, "grep requires --search");
		return 2;
	}

	if (!out->dir_path || !*out->dir_path) {
		set_errbuf(errbuf, errbuf_len, "grep requires --path");
		return 2;
	}

	if (out->dir_path[0] != '/') {
		set_errbuf(errbuf, errbuf_len, "grep requires an absolute directory path");
		return 2;
	}
	...
	if (!S_ISDIR(st.st_mode)) {
		set_errbuf(errbuf, errbuf_len, "grep requires a directory path: %s", out->dir_path);
		return 2;
	}
...
int ela_grep_run(const struct ela_grep_request *request,
		 ...
		 char *errbuf, size_t errbuf_len)
{
	const struct ela_grep_run_ops *effective_ops = ops ? ops : &default_run_ops;
	struct output_buffer buf = {0};
	char *upload_uri = NULL;
	char post_err[256];
	int ret = 0;

	if (!request || !request->search || !request->dir_path)
		return 1;

	if (grep_directory(request, effective_ops, &buf, request->dir_path) != 0) {
		ret = 1;
		goto out;
	}

	if (request->output_uri) {
		upload_uri = effective_ops->build_upload_uri_fn(request->output_uri, "grep", request->dir_path);
		if (!upload_uri) {
