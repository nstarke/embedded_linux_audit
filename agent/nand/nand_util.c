				   ...
				   char *errbuf, size_t errbuf_len)
{
	...
	if (!selected || (!candidates && count)) {
		if (errbuf && errbuf_len)
			snprintf(errbuf, errbuf_len, "Invalid NAND dump candidate list");
		return -1;
	}
	if (!count) {
		if (errbuf && errbuf_len)
			snprintf(errbuf, errbuf_len, "No NAND MTD devices were found");
		return -1;
	}
	for (i = 0; i < count; i++) {
	...
	if (!ties) {
		if (errbuf && errbuf_len)
			snprintf(errbuf, errbuf_len,
				 "NAND MTD devices have no readable size");
		return -1;
	}
	if (ties > 1) {
		if (errbuf && errbuf_len)
			snprintf(errbuf, errbuf_len,
				 "Multiple NAND MTD devices share the largest size (%llu bytes)",
				...
				char *errbuf, size_t errbuf_len)
{
	...
	if (!text || !*text || !index || text[0] < '0' || text[0] > '9') {
		if (errbuf && errbuf_len)
			snprintf(errbuf, errbuf_len, "Invalid NAND device index");
		return -1;
	}
	errno = 0;
	...
	if (errno || end == text || *end || value > UINT32_MAX) {
		if (errbuf && errbuf_len)
			snprintf(errbuf, errbuf_len,
				 "Invalid NAND device index: %s", text);
		return -1;
	}
	*index = (size_t)value;
	return 0;
