...
function createUploadHandler(deps) {
  ...
  return async function uploadHandler(req, res) {
    ...
    if (normalizedContentType === 'application/json' && !['arch', 'cmd', 'module-buildinfo', 'module-vermagic', 'tpm2-getcap', 'tpm2-pcrread', 'tpm2-nvreadpublic', 'tpm2-createprimary'].includes(uploadType)) {
