...
create_web_test_layout() {
    ...
    mkdir -p "$TEST_WEB_TMPDIR/assets" "$TEST_WEB_TMPDIR/data/env" "$TEST_WEB_TMPDIR/tests"
    ...
    printf '#!/bin/sh\necho test-one\n' > "$TEST_WEB_TMPDIR/tests/test_one.sh"
    printf '#!/bin/sh\necho test-two\n' > "$TEST_WEB_TMPDIR/tests/test_two.sh"
    chmod +x "$TEST_WEB_TMPDIR/tests/test_one.sh" "$TEST_WEB_TMPDIR/tests/test_two.sh"
    mkdir -p "$TEST_WEB_TMPDIR/tests/not_a_file"
